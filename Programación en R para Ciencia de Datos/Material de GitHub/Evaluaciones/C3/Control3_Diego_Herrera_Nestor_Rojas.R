...
#Nombre1: Diego Alejandro Herrera Villalobos
#Nombre2: Néstor Patricio Rojas Ríos
...
carpeta_de_trabajo_mac <- paste0(
  '~/Documents/Diplomado_Big_Data_Data_Science/Programación en R para Ciencia ',
  'de Datos/Material de GitHub/Evaluaciones/C3'
)

setwd(carpeta_de_trabajo_linux)
setwd(carpeta_de_trabajo_windows)
setwd(carpeta_de_trabajo_mac)
...
grafico11 <- gapminder %>% 
  ggplot() +
  aes(x = gdpPercap, y = lifeExp, size = pop, color = year) +
  geom_point() +
  scale_size_area() +
  xlab("gdpPercap") +
  ylab("lifeExp") +
...
#descripción de la distribución de la expectativa de vida lifeExp.
...
respuesta15b3 <- paste(
  ...
  'de la Polinesia. También podría ser un factor la presencia de datos',
  'outlayer en Oceanía en comparación a la data de otros continentes.'
)
...
# P6) Considerando el continente de África, y asumiendo normalidad en el
...
print(paste0(
  ...
  round((respuesta24b * 100), 2), # 16.48
  '%. Personalmente, no recomendaría este recorte del personal, pues implica ',
  'que en una de cada seis transacciones el tiempo la espera será excesivo, ',
...
grafico_respuesta26 <- respuesta26 %>% 
  ...
  aes(
    x = hour_band,
    y = selled_items,
    fill = reorder(Item, selled_items, decreasing = TRUE)
  ) +
  geom_col(position = "dodge", colour = '#000000') +
  theme(
...
print(paste(
  ...
  'Pastry o Medialuna, pues tienen lift mayor a 1 y la diferencia entre',
  ' support y confidence no es muy grande. Para el horario entre las',
  '12:00 y las 16:59 horas podría recomendar la asociación de Coffe con Cake o',
  'Sandwich por razones similares a las presentadas en el bloque anterior.',
  'Finalmente, para el horario entre las 17:00 y las 23:59 horas, el candidato',
  'natural es la asociación entre Coffe y Cake, pues presenta los mejores',
  'support, confidence y lift del bloque, aunque también es interesante la',
  'asociación entre Coffe y Alfajores. En general, las promociones',
  'recomendadas se basan en los consecuentes más frecuentes y con mayor',
  'probabilidad condicional, lo que sugiere una fuerte asociación con la',
  'compra de café.'
))
