...
print(paste0(
  ...
  respuesta11d[1, 'food_type'], # chinese
  ' y está presente en ',
  respuesta11d[1, 'city'], # san francisco
  ' y ',
  respuesta11d[2, 'city'], # san leandro
  '.'
))
...
print(paste(
  '1.2a) La ciudad de',
  respuesta12a$city[1], # san jose
  'es la que tiene mayor cantidad de restaurantes sondeados con',
  respuesta12a$restaurants_by_city[1], # 933
  'locales.'
))
...
print(paste0(
  ...
  respuesta12b$food_type[1], # cafe
  ' con ',
  respuesta12b$restaurants_by_food_type[1], # 1098
  ' locales, ',
  respuesta12b$food_type[2], # chinese
  ' con ',
  respuesta12b$restaurants_by_food_type[2], # 1075
  ' locales y ',
  respuesta12b$food_type[3], # pizza
  ' con ',
  respuesta12b$restaurants_by_food_type[3], # 959
  ' locales.'
))
...
print(paste0(
  ...
  ' son ',
  respuesta12c$city[1], # san jose
  ' con ',
  respuesta12c$restaurants_by_city[1], # 28
  ' locales, ',
  respuesta12c$city[2], # berkeley
  ' con ',
  respuesta12c$restaurants_by_city[2], # 14
  ' locales y ',
  respuesta12c$city[3], # oakland
  ' con ',
  respuesta12c$restaurants_by_city[3], # 14
  ' locales.'
))
...
print(paste(
  '1.2d) Tomando las 3 ciudades del ejercicio anterior, escojemos',
  respuesta12d$city[1], # san jose
  'pues tiene la mejor review con',
  round(respuesta12d$mean_review[1], 1), # 2.5
...
print(paste(
  ...
  'es',
  respuesta12e$city[1], # pleasant hill
  'con',
  round(respuesta12e$mean_review[1], 1), # 3.7
  'puntos.'
))
...
print(paste0(
  ...
  respuesta13b$label, # baskin robbins
  ', con locales en ',
  respuesta13b$cities_by_restaurant, # 49
  ' ciudades distintas.'
))
...
respuesta14c <- resumen %>% 
  ...
  labs(
    ...
    caption = paste(
      'Se observa una leve tendencia a que a mayor density food type',
      'existe menor rate review, pero no parece ser una relación significativa.'
    )
  ) +
...
respuesta15b <- respuesta15a %>% 
  ...
  geom_boxplot() +
  labs(
    title = 'Relación entre Tipo de review (alto o bajo) vs Densidad del tipo de comida.',
    x = 'Type Review',
    y = 'Density Food Type'
  ) +
  theme(legend.position = 'none')
...
print(paste(
  '1.5b) El grupo Review alto presenta menor concentración que review bajo.',
  'Existe mayor dispersión de valores outlayer en el grupo de Review alto.'
))
