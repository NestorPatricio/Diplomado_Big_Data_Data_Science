...
datosT <- read.csv("SouthGermanCredit.csv", sep=";",header=TRUE)

# En este ejemplo solo utilizaremos siete variables explicativas, por simplicidad
# Usted deberá usar toda la información

# Elegiremos como variables:
# amount: variable continua
# savings: variable ordinal numerica
# age: variable continua
# housing: variable categorica con 3 niveles
# credits: variable ordinal numerica
# persons: variable binaria
# foreign: variable binaria

datosT_small <- as.data.frame(cbind(amount = datosT$amount,
...
# Considerando la cantidad de niveles en la variable categorica
...
# Usando K = 5
modeloT5 <- kmeans(datosT_norm,
                   centers = 5,
                   nstart = 25)
modeloT5$size

# Agregamos los clusters a cada observacion
datosT_small$cluster3 <- modeloT3$cluster
datosT_small$cluster5 <- modeloT5$cluster


# Valores promedios de variables en cada cluster con K = 3
View(datosT_small
     %>% group_by(cluster3)
     %>% summarise_all(mean))


# Valores promedios de variables en cada cluster con K = 5
View(datosT_small
     %>% group_by(cluster5)
     %>% summarise_all(mean))
